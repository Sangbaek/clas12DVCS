...
class root2pickle():
    ...
    def readEPGG(self, entry_start = None, entry_stop = None, pol = "inbending", detRes = False, logistics = False):
        ...
        if correction:
            ...
            if pol == "outbending":
                FD_phot_corr_marginal =-0.045*(df_gammaRec.loc[df_gammaRec.Gsector<7, "Gp"]-1.5)*(df_gammaRec.loc[df_gammaRec.Gsector<7, "Gp"]-5)/(1+np.exp((df_gammaRec.loc[df_gammaRec.Gsector<7, "Gp"]-4.5)/0.25))/(1+np.exp(-(df_gammaRec.loc[df_gammaRec.Gsector<7, "Gp"]-2)/0.25))
