...
if args.question == 16:
  ...
  path_sim_bkg_1g = '/volatile/clas12/sangbaek/jan2023/q16_18/dset_{}/sim/bkg_1g/{}/'.format(args.plot, polarity)
  path_sim_bkg_2g = '/volatile/clas12/sangbaek/jan2023/q16_18/dset_{}/sim/bkg_2g/{}/'.format(args.plot, polarity)
  
  diet = dvcsvars + ['config', 'Ptheta']
  dsets = []
  for path in [path_exp_epg, path_sim_epg, path_sim_bkg_1g]:
    dfs = []
  ...
  exp_epg, sim_epg, sim_bkg_1g = dsets

  diet = pi0vars + ['config', 'Ptheta']
  dsets = []
  for path in [path_exp_epgg, path_sim_bkg_2g]:
    dfs = []
    for file in glob('{}/*.pkl'.format(path)):
      df = pd.read_pickle(file)
      df = df.loc[:, diet]
