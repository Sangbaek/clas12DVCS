...
e		epgExp.loc[(epgExp.polarity == -1) & (epgExp.config == 2), "unccont{}".format(i)] = epgExp.loc[(epgExp.polarity == -1) & (epgExp.config == 2), "newbin{}".format(i)].map(dict(enumerate(unccontInbCD.flatten())))
	...
	for k in range(kstart, len(collection_xBbins)):
		...
		for xBbin in range(3 - 1):
			for tbin in range(5 - 1):
				...
				axs[5-tbin-2, xBbin].set_xlim([1, 5])
				axs[5-tbin-2, xBbin].set_xticks([1, 2, 3, 4, 5])
				if active == 0:
