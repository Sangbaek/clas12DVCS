				...
				for tind in range(len(tbins)-1):
					for phiind in range(len(phibins)-1):
						...
						for trial in range(n):
							...
							cond = (nub>2 )&(nub<10.604 - 2 ) & (W2(xB, Q2, t1, phi)>4) & (t1>tmin1) & (Kfac2(xB, Q2, t1, phi1)>0)
...
if args.savexsec:
	...
	for k in range(kstart, len(collection_xBbins)):
		...
		for i in range(0, len(collection_cont_xBbins)):
			...
			accCorrectedInb_VGG = divideHist(accCorrectedInb_VGG*histVGGGenInb45nA, histVGGGenInbFD45nA + histVGGGenInbCD45nA+ histVGGGenInbCDFT45nA)
