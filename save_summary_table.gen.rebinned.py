...
def main():
	...
	for model in models:
		...
		for integrated_binnum_gen in range(1, 147+1):
			df = pd.read_pickle("/volatile/clas12/sangbaek/dvcs_related/sim_rad_gen/{}/pkl/fall2018_inb{}/{}.pkl".format(model, string_suffix, integrated_binnum_gen))
			df.loc[:, "integrated_binnum_gen"] = integrated_binnum_gen
			df = df.astype({"integrated_binnum_gen": int, "phi_binnum_gen": int})
			for phi_binnum_gen, phi_width_gen in zip(df_summary_table_rebinned.loc[df_summary_table_rebinned.integrated_binnum == integrated_binnum_gen, "phi_binnum"], df_summary_table_rebinned.loc[df_summary_table_rebinned.integrated_binnum == integrated_binnum_gen, "phi_width"]):
				df_this_bin  = df.loc[(df.phi_binnum_gen >= phi_binnum_gen) & (df.phi_binnum_gen < phi_binnum_gen + phi_width_gen), :]
				directory    = "{}/fall2018_inb{}".format(model, string_suffix)
				n_entry      = len(df_this_bin)
				print("{} {} {}".format(directory, integrated_binnum_gen, phi_binnum_gen))
				if n_entry:
					weight_sum   = np.sum(df_this_bin.GenWeight)
					weight_avg  = np.mean(df_this_bin.GenWeight)
					weight_err  = np.sqrt(np.sum(df_this_bin.GenWeight**2))
					...
					weight_err  = 0#np.sqrt(np.sum(df_this_bin.GenWeight**2))
					...
					n_entry_corrected = 0#np.sum(df_this_bin.weights)
					n_entry_corrected_err = 0
				this_row     = pd.DataFrame([{"integrated_binnum_gen": integrated_binnum_gen, "phi_binnum_gen": phi_binnum_gen, "phi_width_gen": phi_width_gen, "directory": directory, "n_entry": n_entry,
					"bin_volume_gen": bin_volume[integrated_binnum_gen]/24., "n_entry_corrected": n_entry_corrected, "n_entry_corrected_err": n_entry_corrected_err, "weight_sum": weight_sum, "weight_avg": weight_avg, "weight_err": weight_err, "xB_avg": xB_avg,
					"Q2_avg": Q2_avg, "t_avg": t_avg, "phi_avg": phi_avg}])
				df_summary   = pd.concat([df_summary, this_row])


		for integrated_binnum_gen in range(1, 147+1):
			df = pd.read_pickle("/volatile/clas12/sangbaek/dvcs_related/sim_rad_gen/{}/pkl/fall2018_outb{}/{}.pkl".format(model, string_suffix, integrated_binnum_gen))
			df.loc[:, "integrated_binnum_gen"] = integrated_binnum_gen
			df = df.astype({"integrated_binnum_gen": int, "phi_binnum_gen": int})
			for phi_binnum_gen, phi_width_gen in zip(df_summary_table_rebinned.loc[df_summary_table_rebinned.integrated_binnum == integrated_binnum_gen, "phi_binnum"], df_summary_table_rebinned.loc[df_summary_table_rebinned.integrated_binnum == integrated_binnum_gen, "phi_width"]):
				df_this_bin  = df.loc[(df.phi_binnum_gen >= phi_binnum_gen) & (df.phi_binnum_gen < phi_binnum_gen + phi_width_gen), :]
		...
		if model =="pureBH":
			for integrated_binnum_gen in range(1, 147+1):
				df = pd.read_pickle("/volatile/clas12/sangbaek/dvcs_related/sim_rad_gen/{}/pkl/fall2018_inb{}/{}.pkl".format(model, "3", integrated_binnum_gen))
				df.loc[:, "integrated_binnum_gen"] = integrated_binnum_gen
				df = df.astype({"integrated_binnum_gen": int, "phi_binnum_gen": int})
				for phi_binnum_gen, phi_width_gen in zip(df_summary_table_rebinned.loc[df_summary_table_rebinned.integrated_binnum == integrated_binnum_gen, "phi_binnum"], df_summary_table_rebinned.loc[df_summary_table_rebinned.integrated_binnum == integrated_binnum_gen, "phi_width"]):
					df_this_bin  = df.loc[(df.phi_binnum_gen >= phi_binnum_gen) & (df.phi_binnum_gen < phi_binnum_gen + phi_width_gen), :]
	...
	df_summary.to_pickle("/volatile/clas12/sangbaek/clas12DVCS/summary_table.gen.rebinned.pkl")
