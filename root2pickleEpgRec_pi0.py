...
class root2pickle():
    ...
    def save(self, raw = False):
        ...
        df_Rec = self.df_epgg
        df_Rec.loc[:, "closeness"] = np.abs(df_Rec.loc[:, "Mpi0"] - .1349766)
        df_Rec = df_Rec.sort_values(by='closeness', ascending = True)
        df_Rec = df_Rec.loc[~df_Rec.event.duplicated(), ]
        df_Rec = df_Rec.sort_values(by='event')

        pi0anddvcs = df_Rec["event"].isin(self.df_dvcs["event"])
        df_Rec = df_Rec[pi0anddvcs]
