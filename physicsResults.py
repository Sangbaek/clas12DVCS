...
if args.savexsec:
	...
	for k in range(kstart, len(collection_xBbins)):
		...
		for i in range(0, len(collection_cont_xBbins)):
			...
			histExpInbCDFT_plus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 3) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins])
			histExpInbCR_plus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 4) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins])
			histBHDVCSInbFD_plus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 1) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = 1 - epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 1), "contplus{}".format(i)])
			histBHDVCSInbCD_plus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 2) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = 1 - epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 2), "contplus{}".format(i)])
			histBHDVCSInbCDFT_plus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 3) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = 1 - epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 3), "contplus{}".format(i)])
			histBHDVCSInbCR_plus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 4) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = 1 - epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 4), "contplus{}".format(i)])

			histBkgUncInbFD_plus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 1) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 1), "unccontplus{}".format(i)])
			histBkgUncInbCD_plus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 2) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 2), "unccontplus{}".format(i)])
			histBkgUncInbCDFT_plus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 3) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 3), "unccontplus{}".format(i)])
			histBkgUncInbCR_plus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 4) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = epgExp.loc[(epgExp.helicity == -1) & (epgExp.polarity == -1) & (epgExp.config == 4), "unccontplus{}".format(i)])
			...
			histExpInbCDFT_minus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 3) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins])
			histExpInbCR_minus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 4) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins])
			histBHDVCSInbFD_minus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 1) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = 1 - epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 1), "contminus{}".format(i)])
			histBHDVCSInbCD_minus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 2) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = 1 - epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 2), "contminus{}".format(i)])
			histBHDVCSInbCDFT_minus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 3) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = 1 - epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 3), "contminus{}".format(i)])
			histBHDVCSInbCR_minus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 4) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = 1 - epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 4), "contminus{}".format(i)])

			histBkgUncInbFD_minus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 1) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 1), "unccontminus{}".format(i)])
			histBkgUncInbCD_minus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 2) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 2), "unccontminus{}".format(i)])
			histBkgUncInbCDFT_minus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 3) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 3), "unccontminus{}".format(i)])
			histBkgUncInbCR_minus, bins = np.histogramdd(epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 4) , ["xB", "Q2", "t1", "phi1"]].to_numpy(), bins = [xBbins, Q2bins, tbins, phibins], weights = epgExp.loc[(epgExp.helicity == 1) & (epgExp.polarity == -1) & (epgExp.config == 4), "unccontminus{}".format(i)])
