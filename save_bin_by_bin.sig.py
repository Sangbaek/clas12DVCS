...
def main_fast(mode, sig_directory, gen_directory, polarity):
	...
	df_merged_entire.loc[df_merged_entire.weights<1000, :].to_pickle("/volatile/clas12/sangbaek/dvcs_related/{}/excl_level_1/pkl_{}/fall2018_{}.pkl".format(sig_directory, suffix, polarity))
	print("saved")
...
def main_fast_3(mode, sig_directory, gen_directory, polarity):
	...
	df_merged_entire = df_merged_entire.loc[:, df_merged_entire.columns[1:]]
	# polarity = sig_directory.split("_")[-2].split("/")[0]
	df_merged_entire.loc[df_merged_entire.weights<1000, :].to_pickle("/volatile/clas12/sangbaek/dvcs_related/{}/excl_level_1/pkl_3_{}/fall2018_{}.pkl".format(sig_directory, suffix, polarity))
	for integrated_binnum in range(1, 147+1):
		print("Integrated bin number {}".format(integrated_binnum))
		try:
			df_merged[integrated_binnum] = pd.concat(df_merged[integrated_binnum])
...
def main(mode, sig_directory, gen_directory, polarity):
	# if mode == 0:
	# 	main_memory_efficient(mode, sig_directory, gen_directory)
	# else:
	main_fast(mode, sig_directory, gen_directory, polarity)
...
if __name__ == "__main__":
	...
	for mode in [6, 13]:#range(6, 13):#range(11, 12+1):
		...
		main(mode, "sim_rad_rec_fall2018_outb/dvcs_vgg", "dvcs_vgg/fall2018_outb", "outb")
		# main(mode, "sim_rad_rec_fall2018_inb/pureBH", "pureBH/fall2018_inb", "inb")
		main_fast_3(mode, "sim_rad_rec_fall2018_inb/pureBH", "pureBH/fall2018_inb3", "inb")
		main(mode, "sim_rad_rec_fall2018_outb/pureBH", "pureBH/fall2018_outb", "outb")
		# break

	main(13, "sim_rad_rec_fall2018_inb/dvcs_km15", "dvcs_km15/fall2018_inb3_45nA", "inb")
	main(13, "sim_rad_rec_fall2018_outb/dvcs_km15", "dvcs_km15/fall2018_outb3_50nA", "outb")
