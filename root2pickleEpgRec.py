...
class root2pickle():
    ...
    def readEPGG(self, entry_start = None, entry_stop = None, pol = "inbending", gen = "dvcsnorad", detRes = False):
        ...
        df_protonRec.loc[df_protonRec["Psector"]>7, "Pphi"] = df_protonRec.loc[df_protonRec["Psector"]>7, "Pphi"] + np.random.normal(0, 2.0, len(df_protonRec.loc[df_protonRec.Psector>7])) 
        #FD proton
    ...
    def makeDVpi0P_DVCS(self, pol = "inbending"):
        ...
        if pol == 'inbending':
            ...
            cut_mpi02_CDFT = df_dvpi0p["Mpi0"] > 0.116  # mpi0
            cut_mmegg1_CDFT = df_dvpi0p["MM2_egg"] < 2.020  # mmegg
            cut_mmegg2_CDFT = df_dvpi0p["MM2_egg"] > -0.280  # mmegg
            cut_meepgg1_CDFT = df_dvpi0p["ME_epgg"] < 0.941  # meepgg
            cut_meepgg2_CDFT = df_dvpi0p["ME_epgg"] > -0.953  # meepgg
            cut_mpt_CDFT = df_dvpi0p["MPt"] < 0.268  # mpt
            cut_recon_CDFT = df_dvpi0p["reconPi"] < 1.701  # recon gam angle
            cut_mmepgg1_CDFT = np.abs(df_dvpi0p["MM2_epgg"]) < 0.0948  # mmepgg
            cut_mmepgg2_CDFT = np.abs(df_dvpi0p["MM2_epgg"]) > -0.104  # mmepgg
            ...
            cut_mmegg1_CD = df_dvpi0p["MM2_egg"] < 1.947  # mmegg
            cut_mmegg2_CD = df_dvpi0p["MM2_egg"] > -0.0269  # mmegg
            cut_meepgg1_CD = df_dvpi0p["ME_epgg"] < 0.821  # meepgg
            cut_meepgg2_CD = df_dvpi0p["ME_epgg"] > -0.702  # meepgg
            cut_mpt_CD = df_dvpi0p["MPt"] < 0.187  # mpt
            cut_recon_CD = df_dvpi0p["reconPi"] < 1.711  # recon gam angle
            cut_mmepgg1_CD = np.abs(df_dvpi0p["MM2_epgg"]) < 0.0423  # mmepgg
            cut_mmepgg2_CD = np.abs(df_dvpi0p["MM2_epgg"]) > -0.0494  # mmepgg
            ...
            cut_mmegg2_FD = df_dvpi0p["MM2_egg"] > 0.0639  # mmegg
            cut_meepgg1_FD = df_dvpi0p["ME_epgg"] < 0.833  # meepgg
            cut_meepgg2_FD = df_dvpi0p["ME_epgg"] > -0.719  # meepgg
            cut_mpt_FD = df_dvpi0p["MPt"] < 0.199  # mpt
            cut_recon_FD = df_dvpi0p["reconPi"] < 1.481  # recon gam angle
            cut_mmepgg1_FD = np.abs(df_dvpi0p["MM2_epgg"]) < 0.0274  # mmepgg
            cut_mmepgg2_FD = np.abs(df_dvpi0p["MM2_epgg"]) > -0.0313  # mmepgg
            ...
            cut_mmegg1_CDFT = df_dvpi0p["MM2_egg"] < 1.985  # mmegg
            cut_mmegg2_CDFT = df_dvpi0p["MM2_egg"] > -0.248  # mmegg
            cut_meepgg1_CDFT = df_dvpi0p["ME_epgg"] < 0.868  # meepgg
            cut_meepgg2_CDFT = df_dvpi0p["ME_epgg"] > -0.878  # meepgg
            cut_mpt_CDFT = df_dvpi0p["MPt"] < 0.302  # mpt
            cut_recon_CDFT = df_dvpi0p["reconPi"] < 1.553  # recon gam angle
            cut_mmepgg1_CDFT = np.abs(df_dvpi0p["MM2_epgg"]) < 0.116  # mmepgg
            cut_mmepgg2_CDFT = np.abs(df_dvpi0p["MM2_epgg"]) > -0.125  # mmepgg
            ...
            cut_mpi02_CD = df_dvpi0p["Mpi0"] > 0.107  # mpi0
            cut_mmegg1_CD = df_dvpi0p["MM2_egg"] < 1.890  # mmegg
            cut_mmegg2_CD = df_dvpi0p["MM2_egg"] > -0.0316  # mmegg
            cut_meepgg1_CD = df_dvpi0p["ME_epgg"] < 0.710  # meepgg
            cut_meepgg2_CD = df_dvpi0p["ME_epgg"] > -0.642  # meepgg
            cut_mpt_CD = df_dvpi0p["MPt"] < 0.210  # mpt
            cut_recon_CD = df_dvpi0p["reconPi"] < 1.976  # recon gam angle
            cut_mmepgg1_CD = np.abs(df_dvpi0p["MM2_epgg"]) < 0.0380  # mmepgg
            cut_mmepgg2_CD = np.abs(df_dvpi0p["MM2_epgg"]) > -0.0446  # mmepgg
            ...
            cut_mpi01_FD = df_dvpi0p["Mpi0"] < 0.163  # mpi0
            cut_mpi02_FD = df_dvpi0p["Mpi0"] > 0.106  # mpi0
            cut_mmegg1_FD = df_dvpi0p["MM2_egg"] < 1.855  # mmegg
            cut_mmegg2_FD = df_dvpi0p["MM2_egg"] > 0.0110  # mmegg
            cut_meepgg1_FD = df_dvpi0p["ME_epgg"] < 0.804  # meepgg
            cut_meepgg2_FD = df_dvpi0p["ME_epgg"] > -0.674  # meepgg
            cut_mpt_FD = df_dvpi0p["MPt"] < 0.199  # mpt
            cut_recon_FD = df_dvpi0p["reconPi"] < 1.864  # recon gam angle
            cut_mmepgg1_FD = np.abs(df_dvpi0p["MM2_epgg"]) < 0.0309  # mmepgg
    ...
    def makeDVCS(self, pol = "inbending"):
        ...
        if pol == "inbending":
            ...
            cut_mmep1_CDFT = df_dvcs["MM2_ep"] < 0.599  # mmep
            cut_mmep2_CDFT = df_dvcs["MM2_ep"] > -0.528  # mmep
            cut_mmeg1_CDFT = df_dvcs["MM2_eg"] < 1.567  # mmeg
            cut_mmeg2_CDFT = df_dvcs["MM2_eg"] > 0.249  # mmeg
            cut_meepg1_CDFT = df_dvcs["ME_epg"] < 0.480 # meepg
            cut_meepg2_CDFT = df_dvcs["ME_epg"] > -0.431  # meepg
            cut_cone1_CDFT = df_dvcs["coneAngle"] < 29.642  # coneangle
            cut_cone2_CDFT = df_dvcs["coneAngle"] > 12.187  # coneangle
            cut_mpt_CDFT = df_dvcs["MPt"] < 0.0945  # mpt
            cut_recon_CDFT = df_dvcs["reconGam"] < 0.668  # recon gam angle
            cut_coplanarity_CDFT = df_dvcs["coplanarity"] < 8.034  # coplanarity angle
            cut_mmepg1_CDFT = np.abs(df_dvcs["MM2_epg"]) < 0.0121  # mmepg
            ...
            cut_mmep1_CD = df_dvcs["MM2_ep"] < 0.410  # mmep
            cut_mmep2_CD = df_dvcs["MM2_ep"] > -0.401  # mmep
            cut_mmeg1_CD = df_dvcs["MM2_eg"] < 2.311  # mmeg
            cut_mmeg2_CD = df_dvcs["MM2_eg"] > -0.424 # mmeg
            cut_meepg1_CD = df_dvcs["ME_epg"] < 1.065  # meepg
            cut_meepg2_CD = df_dvcs["ME_epg"] > -0.934  # meepg
            cut_cone1_CD = df_dvcs["coneAngle"] < 32.930  # coneangle
            cut_cone2_CD = df_dvcs["coneAngle"] > 12.775  # coneangle
            cut_mpt_CD = df_dvcs["MPt"] < 0.180  # mpt
            cut_recon_CD = df_dvcs["reconGam"] < 0.846  # recon gam angle
            cut_coplanarity_CD = df_dvcs["coplanarity"] < 7.864  # coplanarity angle
            cut_mmepg1_CD = np.abs(df_dvcs["MM2_epg"]) < 0.0237  # mmepg
            cut_mmepg2_CD = np.abs(df_dvcs["MM2_epg"]) > -0.0270  # mmepg
            ...
            cut_mmep1_FD = df_dvcs["MM2_ep"] < 0.553  # mmep
            cut_mmep2_FD = df_dvcs["MM2_ep"] > -0.569  # mmep
            cut_mmeg1_FD = df_dvcs["MM2_eg"] < 2.729  # mmeg
            cut_mmeg2_FD = df_dvcs["MM2_eg"] > -0.924  # mmeg
            cut_meepg1_FD = df_dvcs["ME_epg"] < 1.536  # meepg
            cut_meepg2_FD = df_dvcs["ME_epg"] > -1.468  # meepg
            cut_cone1_FD = df_dvcs["coneAngle"] < 40.881  # coneangle
            cut_cone2_FD = df_dvcs["coneAngle"] > 17.789  # coneangle
            cut_mpt_FD = df_dvcs["MPt"] < 0.333  # mpt
            cut_recon_FD = df_dvcs["reconGam"] < 1.443  # recon gam angle
            cut_coplanarity_FD = 1#df_dvcs["coplanarity"] < 7.822  # coplanarity angle - no cut
            cut_mmepg1_FD = np.abs(df_dvcs["MM2_epg"]) < 0.0553  # mmepg
            cut_mmepg2_FD = np.abs(df_dvcs["MM2_epg"]) > -0.0604  # mmepg
            ...
            cut_Ptheta_CDFT = df_dvcs.Ptheta<60
            cut_Gsector_CDFT = df_dvcs.Gsector>7
            cut_mmep1_CDFT = df_dvcs["MM2_ep"] < 0.617  # mmep
            cut_mmep2_CDFT = df_dvcs["MM2_ep"] > -0.521  # mmep
            cut_mmeg1_CDFT = df_dvcs["MM2_eg"] < 1.613  # mmeg
            cut_mmeg2_CDFT = df_dvcs["MM2_eg"] > 0.215  # mmeg
            cut_meepg1_CDFT = df_dvcs["ME_epg"] < 0.490 # meepg
            cut_meepg2_CDFT = df_dvcs["ME_epg"] > -0.441  # meepg
            cut_cone1_CDFT = df_dvcs["coneAngle"] < 28.865  # coneangle
            cut_cone2_CDFT = df_dvcs["coneAngle"] > 12.512  # coneangle
            cut_mpt_CDFT = df_dvcs["MPt"] < 0.0951  # mpt
            cut_recon_CDFT = df_dvcs["reconGam"] < 0.664  # recon gam angle
            cut_coplanarity_CDFT = df_dvcs["coplanarity"] < 8.235  # coplanarity angle
            cut_mmepg1_CDFT = np.abs(df_dvcs["MM2_epg"]) < 0.0134  # mmepg
            ...
            cut_Ptheta_CD = df_dvcs.Ptheta<60
            cut_Gsector_CD = df_dvcs.Gsector<7
            cut_mmep1_CD = df_dvcs["MM2_ep"] < 0.362  # mmep
            cut_mmep2_CD = df_dvcs["MM2_ep"] > -0.340  # mmep
            cut_mmeg1_CD = df_dvcs["MM2_eg"] < 2.210  # mmeg
            cut_mmeg2_CD = df_dvcs["MM2_eg"] > -0.388 # mmeg
            cut_meepg1_CD = df_dvcs["ME_epg"] < 0.917  # meepg
            cut_meepg2_CD = df_dvcs["ME_epg"] > -0.845  # meepg
            cut_cone1_CD = df_dvcs["coneAngle"] < 7.609  # coneangle
            cut_cone2_CD = df_dvcs["coneAngle"] > 29.370  # coneangle
            cut_mpt_CD = df_dvcs["MPt"] < 0.186  # mpt
            cut_recon_CD = df_dvcs["reconGam"] < 1.010  # recon gam angle
            cut_coplanarity_CD = df_dvcs["coplanarity"] < 7.456  # coplanarity angle
            ...
            cut_Ptheta_FD = df_dvcs.Ptheta>2.477
            cut_Gsector_FD = df_dvcs.Gsector<7
            cut_mmep1_FD = df_dvcs["MM2_ep"] < 0.614  # mmep
            cut_mmep2_FD = df_dvcs["MM2_ep"] > -0.612  # mmep
            cut_mmeg1_FD = df_dvcs["MM2_eg"] < 2.751  # mmeg
            cut_mmeg2_FD = df_dvcs["MM2_eg"] > -0.990  # mmeg
            cut_meepg1_FD = df_dvcs["ME_epg"] < 1.475  # meepg
            cut_meepg2_FD = df_dvcs["ME_epg"] > -1.377  # meepg
            cut_cone1_FD = df_dvcs["coneAngle"] < 38.962  # coneangle
            cut_cone2_FD = df_dvcs["coneAngle"] > 20.075  # coneangle
            cut_mpt_FD = df_dvcs["MPt"] < 0.334  # mpt
            cut_recon_FD = df_dvcs["reconGam"] < 2.010  # recon gam angle
            cut_coplanarity_FD = 1#df_dvcs["coplanarity"] < 7.822  # coplanarity angle - no cut
            cut_mmepg1_FD = np.abs(df_dvcs["MM2_epg"]) < 0.0728  # mmepg
            cut_mmepg2_FD = np.abs(df_dvcs["MM2_epg"]) > -0.0835  # mmepg
