...
class root2pickle():
    ...
    def readEPGG(self, entry_start = None, entry_stop = None, pol = "inbending", gen = "dvcsnorad", detRes = False):
        ...
        #FD proton
        if pol == "inbending":
            df_protonRec.loc[df_protonRec["Psector"]<7, "Pp"] = df_protonRec.loc[df_protonRec["Psector"]<7, "Pp"]*np.random.normal(1, np.abs(2*(0.08)*(1/(1+np.exp(-(df_protonRec.loc[df_protonRec["Psector"]<7, "Pp"]-0.42)/0.05))-0.5)), len(df_protonRec.loc[df_protonRec.Psector<7]))
        elif pol == "outbending":
            df_protonRec.loc[df_protonRec["Psector"]<7, "Pp"] = df_protonRec.loc[df_protonRec["Psector"]<7, "Pp"]*np.random.normal(1, np.abs(2*(0.08)*(1/(1+np.exp(-(df_protonRec.loc[df_protonRec["Psector"]<7, "Pp"]-0.5)/0.075))-0.5)), len(df_protonRec.loc[df_protonRec.Psector<7]))
