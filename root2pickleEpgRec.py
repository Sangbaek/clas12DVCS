...
class root2pickle():
    ...
    def readEPGG(self, entry_start = None, entry_stop = None, pol = "inbending", 
        ...
        if gen == "dvcsrad":
            eleKeysGen.extend(["GenxB", "GenQ2", "Gent", "Genphi", "GenWeight", "BornWeight", "radMode", "helicity", "beamEnergy"])
        ...
        if gen == "dvcsrad":
            if detRes:
                df_electronGen = df_electronGen.loc[:, ["event", "GenEpx", "GenEpy", "GenEpz", "GenEvx", "GenEvy", "GenEvz", "GenxB", "GenQ2", "Gent", "Genphi", "GenWeight", "BornWeight", "radMode", "helicity", "beamEnergy"]]
            else:
                df_electronGen = df_electronGen.loc[:, ["event", "GenEpx", "GenEpy", "GenEpz", "GenxB", "GenQ2", "Gent", "Genphi", "GenWeight", "BornWeight", "radMode", "helicity", "beamEnergy"]]
            df_electronGen = df_electronGen.rename(columns = {"beamEnergy": "beamE"})
        else:
            if detRes:
        ...
        # assign ftof sector to proton sectors
        df_protonRec.loc[df_protonRec.Pstat<4000, "Psector"] = -1000
        df_protonRec.loc[df_protonRec.PFtof1aHitx > -1000, "Psector"] = determineSector(df_protonRec.loc[df_protonRec.PFtof1aHitx > -1000, "PFtof1aHitx"], df_protonRec.loc[df_protonRec.PFtof1aHitx > -1000, "PFtof1aHity"])
    ...
    def makeDVCS(self, pol = "inbending", nofid = False, allowsamesector = False, allowduplicates = False):
        ...
        if allowsamesector:
            ...
            cut_Psector = (~( ((df_dvcs["Pstat"]//10)%10>0) & (df_dvcs["Psector"]==df_dvcs["Gsector"])))  & (df_dvcs["Pstat"]//100%10>0)
    ...
    def save(self, raw = False, pol = "inbending", gen = 'dvcsnorad', efficiency = False):
        ...
        if efficiency:
            df_Rec = assign_efficiency(df_Rec, mc = True, pol = pol)
