...
class root2pickle():
    ...
    def readEP(self, entry_stop = None, pol = "inbending"):
        ...
        if pol == "inbending":
            ...
            AndreyPp_1 = df_protonRecFD_1.loc[:, "Pp"] + np.exp(-2.739*np.ones(len(df_protonRecFD_1.loc[:, "Pp"])) - 3.932*df_protonRecFD_1.loc[:, "Pp"]) + 0.002907;
            AndreyPp_2 = df_protonRecFD_2.loc[:, "Pp"] + np.exp(-1.2*np.ones(len(df_protonRecFD_2.loc[:, "Pp"])) - 4.228*df_protonRecFD_2.loc[:, "Pp"]) + 0.007502;
