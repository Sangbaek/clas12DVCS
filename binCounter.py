...
class binCounter():
	def __init__(self, df):
		self.df = df
		binedges_xB = [0.26, 0.29, 0.32, 0.35, 0.38] # start with small range of xB to ignore finite bin volume effects.
		binedges_Q2 = [2.2, 2.7, 3.2] # start with small range of xB to ignore finite bin volume effects.
		binedges_t = [0.13, 0.18, 0.23, 0.3, 0.39, 0.52, 0.72, 1.1]
	...
	def countinOneBin(self, lower_xB, upper_xB, lower_Q2, upper_Q2, lower_t, upper_t, lower_phi, upper_phi):
		...
		return len(df[cond_lower_xB & cond_upper_xB & cond_lower_Q2 & cond_upper_Q2 & cond_lower_phi & cond_upper_phi & cond_lower_t & cond_upper_t])
...
if __name__ == "__main__":
    ...
    if args.detector == "all":
    	...
    	df = df[(df.Psector>6) & (df.Gsector<7)]
